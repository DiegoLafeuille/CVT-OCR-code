...
class OCR_GUI:
    ...
    def __init__(self, master):
        ...
        # Setup left frame components
        self.setup_general_params_frame()
        self.setup_image_canvas()
        self.setup_button_container()

        # Setup right frame components
        self.setup_parameters_frame()
        self.setup_video_feed()
        self.setup_list_container()

        self.master.update()

    def get_screen_dimensions(self):
        """Get screen width and height, adjust frame and canvas sizes accordingly."""
    ...
    def create_left_frame(self):
        """Create the left frame and place it."""
        self.left_frame = tk.Frame(self.master, width=self.canvas_max_width)
        self.left_frame.place(x=0, y=0, width=self.canvas_max_width, relheight=1)

    def create_right_frame(self):
        """Create the right frame and place it."""
        self.right_frame = tk.Frame(self.master, width=self.right_frame_width)
        self.right_frame.place(x=self.canvas_max_width, y=0, width=self.right_frame_width, relheight=1)

    def setup_general_params_frame(self):
        """Setup the general parameters frame and its components."""
        ...
        self.setup_camera_type_radiobuttons()
        self.setup_daheng_settings_frame()

    def setup_camera_type_radiobuttons(self):
        """Setup the camera type radio buttons."""
        ...
        # See if can be refactored somewhere where it makes more sense
        self.device_manager = gx.DeviceManager()
        self.selected_cam_type = self.cam_type.get()

    def setup_daheng_settings_frame(self):
        """Setup the Daheng camera settings."""
        self.daheng_settings_frame = tk.Frame(self.general_params_frame)
        self.daheng_settings_frame.grid(row=1, column=0, columnspan=3)

        self.create_exposure_time_widgets()
        self.create_gain_widgets()
        self.create_autobalance_button()

    def create_exposure_time_widgets(self):
        """Create widgets for setting exposure time."""
        ...
        self.exposure_time_entry.bind("<Return>", lambda event: self.on_exposure_entry())

        self.exposure_time_label.grid(row=0, column=0, padx=5, pady=(5, 5))
        self.exposure_time_entry.grid(row=0, column=1, padx=5, pady=(5, 5))

    def on_exposure_entry(self):
        """Handle exposure time entry."""
        try:
    ...
    def create_gain_widgets(self):
        """Create widgets for setting gain."""
    ...
    def create_autobalance_button(self):
    ...
    def setup_image_canvas(self):
        """Setup the image canvas for the corrected view of the target surface."""
        ...
        # Presing on canvas draws a rectangle
        self.canvas.bind("<ButtonPress-1>", self.on_button_press)
        self.canvas.bind("<B1-Motion>", self.on_move_press)
        self.canvas.bind("<ButtonRelease-1>", self.on_button_release)

    def setup_button_container(self):
        """Setup the button container for actions."""
    ...
    def setup_parameters_frame(self):
        """Setup the parameters frame and its components."""
        ...
        self.create_camera_input_dropdown()
        self.create_calibration_dropdown()
        self.create_frequency_entry()
        self.create_aruco_dropdown()
        self.create_charuco_board_entries()

    def setup_video_feed(self):
        """Setup the video feed label."""
        self.video_label = tk.Label(self.right_frame)
        self.video_label.pack()
        self.show_camera_feed()
        self.show_perspective_corrected_feed()

    def setup_list_container(self):
        """Setup the list container for rectangle variables."""
    ...
    # Function to create camera input dropdown menu
    def create_camera_input_dropdown(self):
    ...
    # Function to create camera calibration dropdown menu
    def create_calibration_dropdown(self):
    ...
    # Function to create frequency entry
    def create_frequency_entry(self):
        ...
        self.frequency_entry.bind("<Return>", lambda event: self.on_frequency_entry())

    def on_frequency_entry(self):
        try:
            self.frequency = int(self.frequency_entry.get())
        except ValueError:
            messagebox.showerror("Error", "Number of seconds between pictures must be an integer.")

    # Function to create Aruco dropdown menu
    def create_aruco_dropdown(self):
        ...
        self.aruco_dropdown.bind("<<ComboboxSelected>>", lambda event: self.update_aruco())

    # Function to create Charuco board entries (size and dimensions)
    def create_charuco_board_entries(self):
        ...
        self.square_size_entry.bind("<Return>", lambda event: self.on_square_size_entry())
        ...
        self.charuco_width_entry.bind("<Return>", lambda event: self.on_charuco_width_entry())
        ...
        self.charuco_height_entry.bind("<Return>", lambda event: self.on_charuco_height_entry())

    def on_square_size_entry(self):
        try:
    ...
    def on_charuco_width_entry(self):
        try:
    ...
    def on_charuco_height_entry(self):
        try:
