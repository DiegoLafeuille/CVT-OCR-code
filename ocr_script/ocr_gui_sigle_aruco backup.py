...
class OCR_GUI:
    ...
    def indicate_surface_gui(self):
        ...
        self.surface_coords = []
        self.saved_coords = ttk.Label(self.window_feed, text=f"Number of saved surfaces: {len(self.surface_coords)} / 2")
        self.saved_coords.pack()
        ...
        confirm_button.pack(side="left", fill=tk.BOTH, expand=tk.YES, pady=10)

        # Create "Cancel" button
        cancel_button = tk.Button(self.window_feed, text="Cancel", command=self.window_feed.destroy)
        cancel_button.pack(side="right", fill=tk.BOTH, expand=tk.YES, pady=10)
        ...
        if np.all(ids is not None):
            ...
            for id in ids:  
                ...
                if id == 1:
                    ...
                    rvec, tvec, _ = aruco.estimatePoseSingleMarkers(corners[index], self.aruco_size, self.mtx, self.dist)
                    (rvec - tvec).any()  # get rid of numpy value array error
                    cv2.drawFrameAxes(frame, self.mtx, self.dist, rvec, tvec, 0.1, 3)
    ...
    def save_coords(self):
        if len(self.coords) != 4:
            ...
            self.saved_coords.config(text=f"Number of saved surfaces: {len(self.surface_coords)} / 2")
            self.indic_surf_canvas.delete(self.finished_shape)
            self.coords = []
